...
def main():
    ...
    parser.add_argument('--lam1', type=float, default=0.01,
    ...
    for e in range(1, args.epoch + 1):
        ...
        for i, (content, style) in tqdm(enumerate(train_loader, 1)):
            ...
            loss = model(content, style, args.patch_size, args.alpha, args.lam1, args.lam2)
            ...
            if i % args.snapshot_interval == 0:
                ...
                style = style.to(device)
                with torch.no_grad():
                    out = model.generate(content, style, args.patch_size, args.alpha)
